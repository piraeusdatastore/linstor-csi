...
func (n *NfsExporter) Export(ctx context.Context, info *volume.Info, port uint16, params *volume.Parameters) (*url.URL, error) {
	if !n.Enabled() {
		...
		Promoters: []PromoterConfig{{ResourceConfig: map[string]ResourceConfig{
			info.ID: {
				...
				Start: []string{
					...
					fmt.Sprintf("ocf:heartbeat:Filesystem fs_%s_recovery device=/dev/drbd/by-res/%s/1 directory=/var/lib/nfs/ganesha/%s fstype=%s run_fsck=no", info.ID, info.ID, info.ID, info.FsType),
