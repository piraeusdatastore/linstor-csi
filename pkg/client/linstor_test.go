...
//go:generate go run github.com/vektra/mockery/v2@v2.53.5 --srcpkg github.com/LINBIT/golinstor/client --all
...
func TestLinstor_Status(t *testing.T) {
	tcases := []struct {
		...
		{
			// Diskful storage backend + diskless CSI attachment (with property) — only diskless is published
			name:              "pvc-with-diskless-attach",
			response:          []byte(`[{"name":"pvc-with-diskless-attach","node_name":"storage-0","props":{"StorPoolName":"data-pool"},"layer_object":{"children":[{"type":"STORAGE","storage":{"storage_volumes":[{"volume_number":0,"device_path":"/dev/data-vg/pvc-with-diskless-attach_00000","allocated_size_kib":421,"usable_size_kib":1052672,"disk_state":"[]"}]}}],"type":"DRBD","drbd":{"drbd_resource_definition":{"peer_slots":7,"al_stripes":1,"al_stripe_size_kib":32,"port":7010,"transport_type":"IP","secret":"secret","down":false},"node_id":0,"peer_slots":7,"al_stripes":1,"al_size":32,"drbd_volumes":[{"drbd_volume_definition":{"volume_number":0,"minor_number":1010},"device_path":"/dev/drbd1010","backing_device":"/dev/data-vg/pvc-with-diskless-attach_00000","allocated_size_kib":1048840,"usable_size_kib":1048576}],"connections":{"compute-0":{"connected":true,"message":"Connected"}},"promotion_score":10101,"may_promote":true}},"state":{"in_use":false},"uuid":"00000000-0000-0000-0000-000000000001","create_timestamp":1655106116603,"volumes":[{"volume_number":0}]},{"name":"pvc-with-diskless-attach","node_name":"compute-0","props":{"StorPoolName":"DfltDisklessStorPool"},"flags":["DISKLESS","DRBD_DISKLESS"],"layer_object":{"children":[{"type":"STORAGE","storage":{"storage_volumes":[{"volume_number":0,"allocated_size_kib":0,"usable_size_kib":1048576}]}}],"type":"DRBD","drbd":{"drbd_resource_definition":{"peer_slots":7,"al_stripes":1,"al_stripe_size_kib":32,"port":7010,"transport_type":"IP","secret":"secret","down":false},"node_id":1,"peer_slots":7,"al_stripes":1,"al_size":32,"flags":["DISKLESS","INITIALIZED"],"drbd_volumes":[{"drbd_volume_definition":{"volume_number":0,"minor_number":1010},"device_path":"/dev/drbd1010","allocated_size_kib":-1,"usable_size_kib":1048576}],"connections":{"storage-0":{"connected":true,"message":"Connected"}},"promotion_score":10101,"may_promote":true}},"state":{"in_use":true},"uuid":"00000000-0000-0000-0000-000000000002","create_timestamp":1655106118277,"volumes":[{"volume_number":0,"props":{"Aux/csi-created-for":"temporary-diskless-attach"},"provider_kind":"DISKLESS"}]}]`),
			expectedNodes:     []string{"compute-0"},
			expectedCondition: &csi.VolumeCondition{Abnormal: false, Message: "Volume healthy"},
		},
		{
			// Diskful + non-tie-breaker diskless WITHOUT CSI property — diskless is NOT published
